...
def load_valid_tickers_from_csv(csv_path: str) -> Set[str]:
    ...
    valid = df[(df['company_name'].notna()) & (df['company_name'] != '')]['ticker'].dropna().unique()
...
def remove_invalid_tickers(csv_path: str, db_path: str, txt_path: str) -> int:
    ...
    if os.path.exists(db_path):
        ...
        cur.execute(f"DELETE FROM stocks WHERE ticker NOT IN ({placeholders})", list(valid_tickers))
        conn.commit()
